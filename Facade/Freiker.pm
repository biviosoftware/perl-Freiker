...
my($_SELF) = __PACKAGE__->new({
	...
	['FreikerCode.freiker_code' => [
	    ...
	    EXISTS => q{The vs_fe('label'); is already assigned to a participant.  This may not be the right number.  If you are sure the number is correct, vs_wheel_contact();.If(['->is_substitute_user'], SPAN_sui(' To force the assignment, resubmit the form.'));},
	    MUTUALLY_EXCLUSIVE => q{The vs_fe(q{label}); was scanned on vs_fe(q{detail}); when your participant also rode with his old vs_fe(q{label});.  This may not be the right number.  If you are sure the number is correct, vs_wheel_contact();.If(['->is_substitute_user'], SPAN_sui(' To force the assignment, resubmit the form.'));},
	]],
	...
	['Ride.ride_date.EXISTS' => q{The participant was already credited for this date.  Please enter a different date.}],
	...
	['FreikerSelectForm.FreikerCode.freiker_code.PERMISSION_DENIED' => q{Participant is not at this school.}],
	...
	['GreenGearForm.GreenGear.begin_date.EXISTS' => q{All eligible participants have already won at least once.  Uncheck the "prior winners" box to select one of these participants, or change the date range.}],
	['GreenGearForm.GreenGear.begin_date.UNSUPPORTED_TYPE' => q{None of the eligible participants is registered.  Uncheck the "must be registered" box to select one of these participants, or change the date range.}],
	...
	[CLUB_FREIKER_LIST => ['?/participants', '?/freikers']],
	[CLUB_FREIKER_LIST_CSV => ['?/participants.csv', '?/freikers.csv']],
	...
	[CLUB_FREIKER_SELECT => '?/select-participant'],
	[CLUB_FREIKER_PRIZE_SELECT => '?/select-prize'],
	[CLUB_FREIKER_PRIZE_CONFIRM => '?/confirm-prize'],
	[FAMILY_FREIKER_ADD => '?/register-participant'],
	[FAMILY_FREIKER_CODE_ADD => '?/add-tag'],
	[FAMILY_FREIKER_EDIT => '?/edit-participant'],
	[FAMILY_FREIKER_LIST => '?/participants'],
...
#TODO:	    [support_phone => '(800) 555-1212'],
	...
	[ClubRegisterForm => [
	    ...
	    'club_size.desc' => 'Total number of students including participants and non-participants.',
	...
	[[qw(AdmPrizeCouponList ClubPrizeCouponList)] => [
	    ...
	    'Prize.name' => 'Prize',
	    'RealmOwner.display_name' => 'Participant',
	...
	[GreenGear => [
	    ...
	    must_be_registered => 'Participant must be registered in order to win.',
	]],
	[GreenGearForm => [
	...
	[PrizeCouponRedeemForm => [
	    prose => [
		prologue => q{Please enter the Participant's vs_text_as_prose('PrizeReceipt.coupon_code');.},
	    ],
	    ok_button => 'Redeem',
	]],
	[PrizeRideCount => [
...
EOF
	...
	[[qw(FreikerForm FreikerCodeForm)] => [
	    prose => [
		prologue => q{Enter the new Participant ID from the tag on your participant's helmet or backpack.  If the tag is missing from your participant's helmet or backpack, or you need another tag for a new helmet or backpack, vs_wheel_contact();.},
	    ],
	    ...
	    [qw(miles kilometers)] => [
		desc => q{One-way distance from your home to this participant's school.  Participants with multiple homes, calculate the distance from the home associated with the postal code above.},
	    ],
	    ok_button => q{If([qw(Model.FreikerCodeForm FreikerCode.user_id)], 'Add tag', 'Register participant');},
	]],
	[FreikerForm => [
...
EOF
	...
	[[qw(FreikerList ClubFreikerList)] => [
	    ...
	    'RealmOwner.display_name' => 'Participant',
	    empty_list_prose => 'No Participants as yet.',
	]],
	...
	[ClubRideFillForm => [
	    'prose.prologue' => 'This form is used to fill in trips for days when the Dero ZAP did not operate.  Trips will be added for all Participants who rode on two days before and after the date you specified.',
	...
	[prose => [
	    ...
	    USER_CREATE => q{Participant not registered?  vs_link('Click here to register.', 'USER_CREATE');},
	...
	[acknowledgement => [
	    ...
	    FAMILY_FREIKER_ADD => q{Your participant has been added.},
	    FAMILY_MANUAL_RIDE_FORM => q{The missing date has been added.},
	    FAMILY_FREIKER_CODE_ADD => q{The new Participant ID was added.},
	    FAMILY_FREIKER_EDIT => q{The Participant's info was updated.},
...
EOF
	    ...
	    CLUB_FREIKER_PRIZE_CONFIRM => q{Please give the participant the selected prizes.},
	]],
	[xlink => [
	    paypal => 'PayPal',
	...
	[title => [
	    ...
	    CLUB_FREIKER_LIST => 'Participants',
	    ...
	    FAMILY_FREIKER_ADD => 'Register Your Participant',
	    FAMILY_FREIKER_LIST => "Your Participants",
