...
my($_SELF) = __PACKAGE__->new({
	...
	[CLUB_FREIKER_CLASS_LIST_FORM => ['?/kids-class', '?/freikers-class']],
	[CLUB_FREIKER_LIST_CSV => ['?/kids.csv', '?/freikers.csv']],
	[CLUB_FREIKER_BY_CLASS_LIST => ['?/kids-by-class', '?/freikers-by-class']],
...
EOF
	...
	[title => [
	    ...
	    CLUB_FREIKER_BY_CLASS_LIST => q{Kids in String([qw(Model.ClubFreikerByClassList ->get_class_display_name)]);},
